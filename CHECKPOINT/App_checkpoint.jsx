...
const [restTimer, setRestTimer] = useState(0); // secondes restantes
...
useEffect(() => {
  if (screen === "day" && selectedDay) {
    const dayData = planning[selectedDay];
    if (Array.isArray(dayData) && dayData.length > 0) {
      // on clone les exos avec leur état actuel de done depuis le planning
      setPlayWorkout(dayData.map(e => ({ ...e })));
    }
  }
}, [screen, selectedDay, planning]);
...
if (screen === "day") {
  ...
  const completeExo = (i) => {
    const copy = [...playWorkout];
    copy[i].done = true;
    setPlayWorkout(copy);
    playCheck();
    setAnimatedIndex(i);
    setTimeout(() => setAnimatedIndex(null), 300);

    // 🔹 Lancer le timer de repos si défini
    const restSec = copy[i].rest || 0;
    if (restSec > 0) {
    ...
    if (copy.every(e => e.done)) {
      ...
      setActionMsg(""); // vide l'autre message
      setXpAnim(true);
      setPlanning(prev => ({ ...prev, [selectedDay]: copy }));
      ...
      <div style={styles.workoutCard}>
        ...
        <div style={{ marginTop: 20, display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          <PopButton
            style={styles.btn}
            onClick={() => {
              setCustomWorkout(Array.isArray(dayData) ? dayData : emptyWorkout);
      ...
      {timerActive && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 50, marginTop: 10 }}>🛌</div> {/* emoji de repos */}
          </div>
        </div>
      )}
    </div>
...
const applyToAllSameWeekdays = () => {
  ...
  const clean = customWorkout.filter(e => e.exo && e.sets && e.reps);
